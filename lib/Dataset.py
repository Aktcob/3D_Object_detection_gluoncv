...
class Dataset():
    ...
    def __getitem__(self, index):
        ...
        img, truth_orient, truth_conf, truth_dim = nd.array(obj.img,cpu(0)), \
                                                nd.array(label['Orientation'],cpu(0)),\
                                                nd.array(label['Confidence'],cpu(0)),\
                                                nd.array(label['Dimensions'],cpu(0))
        return img, truth_orient, truth_conf, truth_dim
...
class DetectedObject:
    ...
    def calc_theta_ray(self, img, box_2d, proj_matrix):
        ...
        if dx < 0:
            mult = -1
        dx = abs(dx)
        angle = np.arctan(dx/proj_matrix[0][0])
        angle = angle * mult
        return angle
    ...
    def format_img(self, img, box_2d):
        ...
        crop = nd.array(crop,cpu(0))
        # recolor, reformat
        batch = transform_fn(crop)
